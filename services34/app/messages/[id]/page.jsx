  ...
  return (
    <div className="mx-auto flex h-[70vh] max-w-2xl flex-col rounded-lg border border-slate-200 bg-white">
        ...
        {messages.map((m) => {
          ...
          return (
            <div key={m.id} className={`flex ${mine ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[75%] rounded-lg px-3 py-2 text-sm ${mine ? 'bg-brand text-white' : 'bg-slate-100 text-ink'}`}>
                {m.content}
              </div>
            </div>
          );
      ...
      <form onSubmit={send} className="flex gap-2 border-t border-slate-200 p-3">
        ...
        />
        <button className="rounded-md bg-brand px-4 py-2 text-sm font-medium text-white hover:opacity-90">Envoyer</button>
      </form>
    </div>
  );
}
