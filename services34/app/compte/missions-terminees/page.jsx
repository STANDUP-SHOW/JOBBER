...
export default function MissionsTermineesPage() {
  ...
  useEffect(() => {
    if (!authLoading && !user) router.push('/auth/login?next=/compte/missions-terminees');
  ...
  return (
    <div className="mx-auto max-w-xl">
      ...
      <h1 className="mt-4 font-display text-2xl font-semibold text-ink">Missions terminées</h1>
        ...
        {bookings.map((b) => (
          <div key={b.id} className="rounded-lg border border-slate-200 bg-white p-5">
            <div className="flex items-center justify-between">
              ...
              <span className="rounded-full bg-brand/10 px-3 py-1 text-xs font-medium text-brand">Terminée</span>
            </div>
            ...
            {!b.review && (
              <div className="mt-4 rounded-md bg-slate-50 p-4">
                ...
                <textarea
                  ...
                  className="mt-2 w-full rounded-md border border-slate-200 px-3 py-2 text-sm outline-none focus:border-brand"
                />
                <button
                  disabled={busyId === b.id}
                  onClick={() => submitReview(b.id)}
                  className="mt-2 rounded-md bg-brand px-4 py-2 text-sm font-medium text-white hover:opacity-90 disabled:opacity-60"
                >
                  {busyId === b.id ? '…' : "Publier l'avis"}
                </button>
              </div>
            )}
            {b.review && <p className="mt-3 text-xs text-brand">Avis déjà publié — merci !</p>}
          </div>
        ))}
      </div>
    </div>
