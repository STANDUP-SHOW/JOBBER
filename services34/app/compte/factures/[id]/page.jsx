...
function invoiceNumber(booking) {
  ...
  return `S34-${year}-${booking.id.slice(-8).toUpperCase()}`;
}
...
export default function FactureDetailPage() {
  ...
  useEffect(() => {
    if (!authLoading && !user) router.push('/auth/login?next=/compte/factures');
  ...
  if (error) {
    return (
      <div className="mx-auto max-w-xl">
        <Link href="/compte/factures" className="text-sm font-medium text-brand">← Mes factures</Link>
    ...
    <div className="mx-auto max-w-2xl">
      ...
      <div className="mt-6 rounded-lg border border-slate-200 bg-white p-8 print:border-0 print:p-0">
        <div className="flex items-start justify-between">
          <div>
            <div className="font-display text-2xl font-bold text-brand">Services 34</div>
            <div className="mt-1 text-xs text-slate-400">services34.fr</div>
          </div>
        ...
        <div className="mt-8 grid grid-cols-2 gap-6">
          <div>
            ...
            <div className="mt-1 text-sm font-medium text-ink">{user.firstName} {user.lastName}</div>
          ...
          <div>
            ...
            <div className="mt-1 text-sm font-medium text-ink">{providerName || 'Prestataire Services 34'}</div>
          </div>
        </div>
        ...
        <p className="mt-8 text-xs text-slate-400">
          Facture générée automatiquement par Services 34 lors du versement au prestataire.
        </p>
      </div>
    </div>
  );
