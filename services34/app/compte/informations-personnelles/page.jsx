...
export default function InformationsPersonnellesPage() {
  ...
  useEffect(() => {
    if (!authLoading && !user) router.push('/auth/login?next=/compte/informations-personnelles');
  ...
  return (
    <div className="mx-auto max-w-xl">
      ...
      <form onSubmit={onSubmit} className="mt-6 space-y-4">
        ...
        {saved && <p className="rounded-md bg-brand/10 px-3 py-2 text-sm text-brand">Informations mises à jour.</p>}

        <button disabled={loading} className="w-full rounded-md bg-brand py-3 font-medium text-white hover:opacity-90 disabled:opacity-60">
      ...
      <form onSubmit={onPasswordSubmit} className="mt-3 space-y-3">
        ...
        <input
          ...
          className="w-full rounded-md border border-slate-200 px-3 py-2 text-sm outline-none focus:border-brand"
        />
        {pwError && <p className="rounded-md bg-red-50 px-3 py-2 text-sm text-red-600">{pwError}</p>}
        {pwSaved && <p className="rounded-md bg-brand/10 px-3 py-2 text-sm text-brand">Mot de passe mis à jour.</p>}
        <button disabled={pwBusy} className="w-full rounded-md border border-slate-200 py-3 font-medium text-ink hover:border-brand disabled:opacity-60">
