...
const GROUPS = [
  ...
  },
  {
    title: 'SMS',
    items: [
      { key: 'notifySmsOffers', label: "J'ai reçu des propositions pour ma demande" },
...
export default function NotificationsPage() {
  ...
  useEffect(() => {
    if (!authLoading && !user) router.push('/auth/login?next=/compte/notifications');
            ...
            {group.items.map((item) => (
              <div key={item.key} className="flex items-center gap-3 px-4 py-3.5">
                ...
                <button
                  ...
                  className={`relative h-6 w-11 shrink-0 rounded-full transition-colors disabled:opacity-50 ${prefs[item.key] ? 'bg-brand' : 'bg-slate-300'}`}
                >
