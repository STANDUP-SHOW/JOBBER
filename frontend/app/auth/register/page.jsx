...
function RegisterForm() {
  ...
  async function onSubmit(e) {
    e.preventDefault();
    setError('');
    if (form.isProfessional && !isValidSiret(form.professionalSiret)) {
      setError('Numéro KBIS/SIRET invalide (14 chiffres).');
      return;
    }
    setLoading(true);
    try {
      const { token, user } = await api.register({
        ...
        professionalSiret: form.isProfessional ? form.professionalSiret : undefined,
  ...
  return (
    <div className="mx-auto max-w-md">
      ...
      <p className="mt-4 text-sm text-slate-500">
        Vous dirigez une entreprise ? <a href="/auth/register-entreprise" className="font-medium text-moss hover:underline">Ouvrir un compte entreprise</a>
      </p>
      ...
      <form onSubmit={onSubmit} className="mt-6 space-y-4">
        ...
        <div className="rounded-lg border border-slate-200 p-4">
          <span className="text-xs font-medium text-slate-500">Êtes-vous professionnel ?</span>
          <div className="mt-2 grid grid-cols-2 gap-3">
            <button
              type="button"
              onClick={() => setForm((f) => ({ ...f, isProfessional: false }))}
              className={`rounded-lg border-2 p-3 text-left text-sm font-semibold transition ${
                !form.isProfessional ? 'border-moss bg-moss-light text-ink' : 'border-slate-200 text-slate-600 hover:border-moss'
              }`}
            >
              Non
            </button>
            <button
              type="button"
              onClick={() => setForm((f) => ({ ...f, isProfessional: true }))}
              className={`rounded-lg border-2 p-3 text-left text-sm font-semibold transition ${
                form.isProfessional ? 'border-moss bg-moss-light text-ink' : 'border-slate-200 text-slate-600 hover:border-moss'
              }`}
            >
          ...
          {form.isProfessional ? (
            <label className="mt-3 block">
              <span className="text-xs font-medium text-slate-500">Numéro KBIS / SIRET</span>
              <input
                type="text"
                inputMode="numeric"
                maxLength={14}
                placeholder="14 chiffres"
                value={form.professionalSiret}
                onChange={(e) => setForm({ ...form, professionalSiret: e.target.value.replace(/\D/g, '') })}
                className="mt-1 w-full rounded-md border border-slate-200 px-3 py-2 text-sm outline-none focus:border-moss"
              />
              {form.professionalSiret && !isValidSiret(form.professionalSiret) && (
                <span className="mt-1 block text-xs text-clay">Numéro invalide.</span>
              )}
            </label>
          ) : (
            <p className="mt-3 text-xs text-slate-500">
              Aucune inquiétude, vous pourrez quand même manager ou jobber !
            </p>
          )}
        </div>
