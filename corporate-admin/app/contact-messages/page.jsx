...
export default function AdminContactMessagesPage() {
  ...
  return (
    <div className="max-w-2xl">
      ...
      <p className="mt-1 text-sm text-slate-500">Formulaire de contact soumis depuis {brand.name}.</p>
      ...
      <ul className="mt-6 divide-y divide-slate-100 rounded-lg border border-slate-200 bg-white">
        {messages.map((m) => (
          <li key={m.id}>
            <Link href={`/contact-messages/${m.id}`} className="flex items-center justify-between gap-3 px-4 py-3.5 hover:bg-slate-50">
