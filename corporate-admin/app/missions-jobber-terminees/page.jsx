...
export default function MissionsJobberTermineesPage() {
  ...
  async function refresh() {
    try {
      const { missions } = await agencyApi.missionsJobberTerminees(token);
  ...
  return (
    <div>
      <h1 className="font-display text-2xl font-semibold text-ink">Missions Jobber terminées</h1>
      {error && <p className="mt-4 rounded-md bg-clay/10 px-3 py-2 text-sm text-clay">{error}</p>}
      {missions === null && <p className="mt-4 text-sm text-slate-400">Chargement…</p>}
      {missions?.length === 0 && <p className="mt-4 text-sm text-slate-400">Aucune mission terminée pour l'instant.</p>}
      ...
      <div className="mt-6 space-y-3">
        {missions?.map((m) => (
          <div key={m.id} className="relative flex items-center justify-between rounded-lg border border-slate-200 bg-white p-4">
            ...
            <div>
              <Link href={`/missions/${m.id}`} className="font-medium text-ink hover:underline">{m.title}</Link>{' '}
