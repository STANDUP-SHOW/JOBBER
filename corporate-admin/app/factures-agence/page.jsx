...
export default function FacturesAgencePage() {
  ...
  async function refresh() {
    try {
      const { invoices } = await agencyApi.invoices('agence', token);
  ...
  async function generateMonthly() {
    setBusy(true);
    try {
      await agencyApi.generateMonthlyInvoice('agence', token);
      await refresh();
  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <h1 className="font-display text-2xl font-semibold text-ink">Mes factures Agence</h1>
      ...
      <p className="mt-1 text-sm text-slate-500">
        Une facture par mission (référence-{brand.shortName.toUpperCase()}) et une facture récapitulative par mois.
      </p>
          ...
          <tbody>
            {invoices?.map((i) => (
              <tr key={i.id} className="border-b border-slate-100 last:border-0">
                ...
                <td className="px-4 py-3 text-slate-500">{i.kind === 'AGENCY_MONTHLY' ? 'Mensuelle' : 'Par mission'}</td>
