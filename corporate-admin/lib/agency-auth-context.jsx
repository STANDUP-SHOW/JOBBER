...
export function AgencyAuthProvider({ children }) {
  ...
  useEffect(() => {
    const saved = typeof window !== 'undefined' ? localStorage.getItem('corporate_agency_token') : null;
    if (saved) {
      setToken(saved);
  ...
  function login(newToken, newAgency) {
    localStorage.setItem('corporate_agency_token', newToken);
    setToken(newToken);
    setAgency(newAgency);
  }

  function logout() {
    localStorage.removeItem('corporate_agency_token');
    setToken(null);
    setAgency(null);
  }
